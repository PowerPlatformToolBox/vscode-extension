...
export class ToolHostPanel {
    ...
    private constructor(panel: vscode.WebviewPanel, extensionUri: vscode.Uri, toolRegistryManager: ToolRegistryManager, toolManager: ToolManager, managers?: OpenManagers, initialToolId?: string) {
        this.panel = panel;
     ...
     */
    static open(extensionUri: vscode.Uri, toolRegistryManager: ToolRegistryManager, toolManager: ToolManager, managers?: OpenManagers, initialToolId?: string): void {
        ...
        const panel = vscode.window.createWebviewPanel("pptb.toolHostPanel", "PPTB Tools", column ?? vscode.ViewColumn.One, {
            enableScripts: true,
            localResourceRoots: [vscode.Uri.joinPath(extensionUri, "dist", "webviews"), vscode.Uri.joinPath(extensionUri, "src", "polyfill"), toolManager.toolsDirUri],
            retainContextWhenHidden: true,
        });

        ToolHostPanel.currentPanel = new ToolHostPanel(panel, extensionUri, toolRegistryManager, toolManager, managers, initialToolId);
    }
    ...
    private async dispatchPowerPlatform(method: string, args: unknown[]): Promise<unknown> {
        ...
        return this.powerPlatformManager.request(namespace, httpMethod, typeof path === "string" ? path : "", body, target, (headers as Record<string, string>) ?? {});
    }
            ...
            if (tool) {
                ...
                for (const candidate of candidates) {
                    ...
                    try {
                        ...
                        const htmlWebviewUri = this.panel.webview.asWebviewUri(vscode.Uri.file(candidate)).toString();
                        const baseUri = htmlWebviewUri.substring(0, htmlWebviewUri.lastIndexOf("/"));
                        ...
                        const injection = `<base href="${baseUri}/">\n` + `<script>\n${polyfillContent}\n</script>`;
    ...
    private getHtmlForWebview(webview: vscode.Webview): string {
        ...
        const polyfillUri = webview.asWebviewUri(vscode.Uri.joinPath(this.extensionUri, "src", "polyfill", "toolboxAPI.js"));
