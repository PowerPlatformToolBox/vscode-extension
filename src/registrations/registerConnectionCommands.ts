...
export function registerConnectionCommands(
    ...
    const editCmd = vscode.commands.registerCommand("pptb.connections.edit", async (treeItem?: ConnectionTreeItem) => {
        ...
        if (!connection) {
            void vscode.window.showWarningMessage("No connection selected to edit.");
            return;
        }
    ...
    const deleteCmd = vscode.commands.registerCommand("pptb.connections.delete", async (treeItem?: ConnectionTreeItem) => {
        const connection = treeItem?.connection;
        if (!connection) {
            void vscode.window.showWarningMessage("No connection selected to delete.");
            return;
        }
        ...
        if (confirm === "Delete") {
            ...
            void vscode.window.showInformationMessage(`Connection "${connection.name}" deleted.`);
        }
    });
    ...
    const testCmd = vscode.commands.registerCommand("pptb.connections.test", async (treeItem?: ConnectionTreeItem) => {
        ...
        try {
            ...
            if (ok) {
                void vscode.window.showInformationMessage(`Connection "${connection.name}" is working.`);
            } else {
                void vscode.window.showErrorMessage(`Connection "${connection.name}" test failed.`);
            }
        } catch (err: unknown) {
            const msg = err instanceof Error ? err.message : String(err);
    ...
    const forgetCmd = vscode.commands.registerCommand("pptb.connections.forget", async (treeItem?: ConnectionTreeItem) => {
        ...
        void vscode.window.showInformationMessage(`Connection "${connection.name}" forgotten — cached tokens cleared.`);
    });
    ...
    const exportCmd = vscode.commands.registerCommand("pptb.connections.export", async () => {
        ...
        void vscode.window.showInformationMessage(`Exported ${exported.connections.length} connection(s) to ${saveUri.fsPath}`);
    });
    ...
    const importCmd = vscode.commands.registerCommand("pptb.connections.import", async () => {
        ...
        try {
            ...
            if (result.warnings.length > 0) {
                void vscode.window.showWarningMessage(`${msg} Warnings: ${result.warnings.join(" ")}`);
            } else {
