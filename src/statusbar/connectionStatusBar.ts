  ...
  constructor(connectionsManager: ConnectionsManager) {
    ...
    this.disposables.push(
      vscode.commands.registerCommand(
        "pptb.statusBar.pickConnection",
        () => this.showConnectionPicker()
      )
    );
  ...
  private async showConnectionPicker(): Promise<void> {
    ...
    if (connections.length === 0) {
      vscode.window.showInformationMessage(
        "No connections saved. Use PPTB: Add Connection to create one."
      );
      return;
    }
    ...
    const items: (vscode.QuickPickItem & { id: string })[] = connections.map(
      (conn) => ({
        id: conn.id,
        label: conn.name,
