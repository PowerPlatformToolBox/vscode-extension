...
export class DataverseManager {
  ...
  async execute(
    ...
    const { operationName, operationType, entityName, entityId, parameters } =
      request;
  ...
  async getAllEntitiesMetadata(
    connection: Connection,
    selectColumns?: string[]
  ): Promise<EntityMetadataCollection> {
  ...
  async getEntityRelatedMetadata(
    connection: Connection,
    entityLogicalName: string,
    relatedPath: string,
    selectColumns?: string[]
  ...
  async associate(
    ...
    const primarySetName = await this.getEntitySetName(
      connection,
      primaryEntityName
    );
    const relatedSetName = await this.getEntitySetName(
      connection,
      relatedEntityName
    );
    const baseURL = `${connection.url}/api/data/${DATAVERSE_API_VERSION}/`;
    ...
    return this.request<Record<string, unknown>>(connection, {
      ...
      data: {
        ...
        OverwriteUnmanagedCustomizations:
          options?.overwriteUnmanagedCustomizations ?? false,
      },
    });
  }
