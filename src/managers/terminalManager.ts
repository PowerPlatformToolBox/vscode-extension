...
export class TerminalManager implements vscode.Disposable {
    ...
    private readonly _onTerminalOutput = new vscode.EventEmitter<TerminalOutputEvent>();
    private readonly _onTerminalClosed = new vscode.EventEmitter<TerminalClosedEvent>();
    private readonly _onTerminalCommandCompleted = new vscode.EventEmitter<TerminalCommandCompletedEvent>();
    private disposables: vscode.Disposable[] = [];

    /** Fired for each output chunk when shell integration is active on the terminal. */
    readonly onTerminalOutput: vscode.Event<TerminalOutputEvent> = this._onTerminalOutput.event;

    /** Fired when a managed terminal is closed (programmatically or by the user). */
    readonly onTerminalClosed: vscode.Event<TerminalClosedEvent> = this._onTerminalClosed.event;
     ...
     */
    readonly onTerminalCommandCompleted: vscode.Event<TerminalCommandCompletedEvent> = this._onTerminalCommandCompleted.event;
     ...
     */
    create(options: TerminalCreateOptions, toolId: string, toolInstanceId: string): TerminalInfo {
        const id = uuidv4();
     ...
     * executed.
     */
    execute(terminalId: string, command: string): { terminalId: string; commandId: string } {
        const entry = this.terminals.get(terminalId);
        if (!entry) {
    ...
    list(): TerminalInfo[] {
        return Array.from(this.terminals.values()).map((e) => this.toTerminalInfo(e));
    }
     ...
     */
    private async streamOutput(terminalId: string, toolInstanceId: string, execution: vscode.TerminalShellExecution): Promise<void> {
        try {
