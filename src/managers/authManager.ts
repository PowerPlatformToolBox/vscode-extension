        ...
        const code = await new Promise<string>((resolve, reject) => {
            const server = http.createServer((req, res) => {
                ...
                if (authCode) {
                    ...
                    res.end(`<html><body><h2>Authentication failed: ${escapeHtml(errorDesc)}</h2></body></html>`);
                    server.close();
